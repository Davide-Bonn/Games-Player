...
function renderGesturePreview(){
  ...
  GESTURE_LIST.forEach(g=>{
    ...
    if(a&&a!=="NONE"){
      ...
      chip.textContent=`${GESTURE_LABELS[g]}  →  ${ACTION_LABELS[a]}`;
      el.appendChild(chip);
    }
  });
}
...
function drawGestureIconCanvas(canvas, gesture, large=false){
  ...
  if(gesture.includes("blink")){
    ...
    ctx.fillStyle="#fff";
    if(lc){ctx.strokeStyle="#007aff";ctx.lineWidth=large?3:2;ctx.beginPath();ctx.moveTo(le-r*.25,ey);ctx.lineTo(le+r*.25,ey);ctx.stroke();}
    else{ctx.beginPath();ctx.arc(le,ey,r*.18,0,Math.PI*2);ctx.fill();ctx.fillStyle="#121216";ctx.beginPath();ctx.arc(le,ey,r*.08,0,Math.PI*2);ctx.fill();}
    ctx.fillStyle="#fff";
    if(rc){ctx.strokeStyle="#007aff";ctx.lineWidth=large?3:2;ctx.beginPath();ctx.moveTo(re-r*.25,ey);ctx.lineTo(re+r*.25,ey);ctx.stroke();}
    else{ctx.beginPath();ctx.arc(re,ey,r*.18,0,Math.PI*2);ctx.fill();ctx.fillStyle="#121216";ctx.beginPath();ctx.arc(re,ey,r*.08,0,Math.PI*2);ctx.fill();}
    ...
    ctx.fillStyle="#007aff";ctx.beginPath();ctx.ellipse(cx,cy+r*.25,r*.25,r*.3,0,0,Math.PI*2);ctx.fill();
    ctx.fillStyle="#121216";ctx.beginPath();ctx.ellipse(cx,cy+r*.25,r*.15,r*.18,0,0,Math.PI*2);ctx.fill();
  } else if(gesture==="mouth_close"){
    ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.stroke();
    ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(cx-r*.3,cy-r*.2,r*.15,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.arc(cx+r*.3,cy-r*.2,r*.15,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle="#007aff";ctx.lineWidth=large?3:2;ctx.beginPath();ctx.moveTo(cx-r*.25,cy+r*.3);ctx.lineTo(cx+r*.25,cy+r*.3);ctx.stroke();
  } else if(gesture==="pinch_index"||gesture==="pinch_middle"){
    const clr=gesture==="pinch_index"?"#007aff":"#5856d6";
